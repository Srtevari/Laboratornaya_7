...
Weather XmlService::getWeather(const string& s)
{
    xml_document doc;
    xml_parse_result result = doc.load_file(s.c_str());
    if (!result)
        throw std::runtime_error("Failed to load XML file: " + s);
    ...
    std::string weather = node.child("weather").attribute("value").as_string(); // "небольшой дождь"
